...
const props = defineProps({
	...
	labelFont : {type : String, default : null},
});
...
<template>
	<template v-if="is_init">
		<div v-if="check" :class="'component-forms-select-lang-w29 group' + (float 
			...
			) + ' ' +(is_boolean(float) ? 'inline-block max-w-full' : ' right-4 top-4')
			: ' relative'
		)">
			<select name="lang" class="ff-jost peer absolute inset-0 m-auto w-full h-full z-[1] opacity-0 overflow-hidden" @change="handleChange">
			...
			<div :class="'content-2eesd flex justify-between items-center py-2 rounded-3xl z-10' + (float
				...
				<label for="lang" class="truncate" :class="{'ff-jost text-sm' : !labelFont, [labelFont] : labelFont, 'transition-all duration-200 max-w-0 ml-0 group-hover:max-w-max group-focus-within:max-w-max group-hover:ml-2 group-focus-within:ml-2' : float}">{{ displayResult(select_value, 'label') }}</label>
